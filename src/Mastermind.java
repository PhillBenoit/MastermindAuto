...
public class Mastermind {
	...
	private static void getGuess() {
		...
		do {
			//get input
			String guess = getString(String.format(GUESS_MESSAGE, turn+1)).toUpperCase();
			
			is_valid = isValidGuess(guess);

			//process input
			if (is_valid) processTurn(guess.toUpperCase());
	...
	static private boolean isValidGuess(String guess) {
		if (guess.length() != MastermindModel.ANSWER_SIZE) return false;
		for (char color : guess.toCharArray())
            if (controller.getColorIndex(color) == -1) return false;
		return true;
	}
